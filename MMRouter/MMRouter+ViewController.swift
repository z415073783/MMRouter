...
@objc public extension MMPushRouter {
    class func push(name: String, target: UINavigationController, animated: Bool, finishBlock: @escaping () ->Void ) {
        
        callMethod(key: kRouterMarkPreNamePush + name, params: [target, animated]) { (_) in
            finishBlock()
        }
    }
    ...
    class func registerPush(target: NSObject, name: String, pushVC: UIViewController.Type) {
...
@objc public extension MMPresentRouter {
    class func present(name: String, target: UIViewController, animated: Bool, finishBlock: @escaping () ->Void ) {
        callMethod(key: kRouterMarkPreNamePush + name, params: [target, animated]) { (_) in
            finishBlock()
        }
    }
    ...
    class func registerPresent(target: NSObject, name: String, presentVC: UIViewController.Type) {
        let model = MMRouterModel(target: target, key: kRouterMarkPreNamePush + name) { (params, finishBlock) in
            guard let list = params as? [Any] else { return }
