...
//    MMRouter
    ...
    if (windowScene != nil) {
        ...
        [FirstModuleRouter registerModule];
        [SecondModuleRouter registerModule];
        [MMRouter callMethodWithKey:@"openFirstVC" params:nil finishBlock:^(UIViewController* vc) {
            [nc pushViewController:vc animated:YES];
            
            [NSTimer scheduledTimerWithTimeInterval:1 repeats:false block:^(NSTimer * _Nonnull timer) {
                [MMRouter callMethodWithKey:@"openSecondVC" params:nil finishBlock:^(UIViewController* vc) {
                    [nc pushViewController:vc animated:YES];
                }];
            }];
